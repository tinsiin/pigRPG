...
public sealed class TurnExecutor
{
    ...
    {
        _eventBus?.Publish(BattleEvent.UiDisplayLogs());
        ...
        _eventBus?.Publish(BattleEvent.UiMoveActionMark(_context.Acter, false, true));
        ...
        if (_context.ActerFaction == allyOrEnemy.alliy)
        {
            ...
            {
                _context.Logger.Log(_context.Acter.CharacterName + "主人公キャラの強制続行スキルがないのでスキル選択へのパッシブ判定処理へと進みます。");
                if (!isFreezeByPassives || hasCanCancelCantACTPassive)
                {
                    ...
                    _eventBus?.Publish(BattleEvent.UiSwitchAllySkillUiState(_context.Acter, hasSingleTargetReservation));
                    _context.Logger.Log(_context.Acter.CharacterName + "(主人公キャラ)はスキル選択");
    ...
    public void NextTurn(bool next)
    {
        ...
        if (next)
        {
            ...
            TurnAdvanced?.Invoke(_context.BattleTurnCount);
