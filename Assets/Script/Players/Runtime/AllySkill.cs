...
public class AllySkill : BaseSkill
{
    ...
    /// </summary>
    public override void DoSkillCountUp(BaseStates actor = null)
    {
        base.DoSkillCountUp(actor);
        AddProficiency(actor);
        AddEmotionalAttachmentSkillQuantity(actor);
    }

    /// <summary>
    /// 熟練度を加算する（思い入れ量に応じてスケーリング）
    /// </summary>
    void AddProficiency(BaseStates actor)
    {
        if (actor is not AllyClass ally) return;
        // このスキルが思い入れスキルかどうかチェック
        if (ally.EmotionalAttachmentSkillID == ID)
        {
            // 思い入れスキルの場合、思い入れ量に応じてスケーリング
            float multiplier = ally.GetProficiencyMultiplierFromQuantity();
            Proficiency += multiplier;
            Debug.Log($"思い入れスキル熟練度加算: +{multiplier:F2} (思い入れ量: {ally.EmotionalAttachmentSkillQuantity})");
        }
        else
        {
            // 通常スキルは1.0倍
    ...
    /// スキルの思い入れ由来の固定加算値
    /// </summary>
    float CurrentHPFixedAdditionSkillPower(BaseStates actor)
    {
        if(actor is not AllyClass ally) return 0;
        if(ally.EmotionalAttachmentSkillID == ID)//思い入れスキルならhp固定加算が入る
        {
            // 思い入れ量から倍率を取得
            float multiplier = ally.GetCurrentHPFixedAdditionMultiplier();

            // 基礎値×現在HP = SkillPower加算値
            float currentHP = ally.HP;
            float additionPower = multiplier * currentHP;

            Debug.Log($"思い入れスキルHP固定加算: +{additionPower:F2} (倍率: {multiplier:F3}, 現在HP: {currentHP}, 思い入れ量: {ally.EmotionalAttachmentSkillQuantity})");
    ...
    /// 思い入れ量を加算する用の関数
    /// </summary>
    void AddEmotionalAttachmentSkillQuantity(BaseStates actor)
    {
        if (actor is AllyClass ally)
            ally.EmotionalAttachmentSkillQuantity++;
    }
    /// <summary>
    ...
    public void ApplyEmotionalAttachmentSkillQuantityChangeSkillWeakeningPassive(BaseSkillPassive passive, AllyClass actor)
    {
        var tuning = actor?.Tuning;
