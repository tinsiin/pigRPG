    ...
    private void Awake()
    {
        if (Instance == null)
        {
            ...
            {//listの{}内に順不同の組み合わせでキーを指定できる。
                { NormalizeSpiritualKey(new List<SpiritualProperty>{SpiritualProperty.baledrival,SpiritualProperty.pysco,SpiritualProperty.sacrifaith}),PartyProperty.HolyGroup}
