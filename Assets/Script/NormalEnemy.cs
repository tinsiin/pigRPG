...
public class NormalEnemy : BaseStates
{
    ...
    public int RecovelyStepCount;//実際のカウント
    ...
    {
        if (RecovelyStepCount <= 0) return false;//既にカウントがゼロなら生きてるってこと。　復活カウントもせずに返す

        var difference = Math.Abs(nowProgress - _lastEncountProgress);//差の絶対値を取得

        if((RecovelyStepCount -= difference) <= 0)//復活までのカウントから(前エリアと今の進行度の差)を引いて0以下になったら
        {
            RecovelyStepCount = 0;//逃げてもまた出てくる　殺さないとまたrecovelyStepは設定されない。
            return true;//復活する。
        }
