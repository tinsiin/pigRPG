...
関連仕様書:
- `doc/スキル/スキル計算仕様書.md` — 四大ステ計算式、SkillPower、ダメージ補正チェーン、命中/回避
...
enum全体は3つのカテゴリに分類される。

### キャラ精神属性（10種）

キャラクターの `DefaultImpression` / `MyImpression` として持つ属性。それぞれ固有の挙動（十日能力係数、死亡時精神HP変動、乖離判定など）を持つ。
...
キャラが取りうる状態ではあるが、精神属性としての固有挙動を持たない。「まだ何にも染まっていない白紙」。

- 精神補正: 100%（無変動）
- 十日能力の成長: 不可（`SpritualTenDayAbilitysMap` にNoneのエントリがない）
- 乖離スキル判定: スキップ
- パーティ属性補正: なし
- 非特殊スキルを受けると**そのスキルの精神属性に染まる**（唯一の動的性質）
- Mvoidスキルで意図的にNoneに戻されることもある

### スキル専用精神属性（4種）

キャラの属性にはならない。スキルの精神属性としてのみ存在し、実行時に被害者・実行者の精神属性を操作する特殊処理を持つ。

| 値 | 日本語 | 精神補正 | 実行者の属性変化 | 被害者の属性変化 |
|---|---|---|---|---|
| `Mvoid` (1<<11) | ムヴォイド | なし（Noneになるため） | Noneになる | 実行者のキャラ属性に上書き |
| `Galvanize` (1<<12) | ガルヴァナイズ | 実行者のキャラ属性で計算 | 変化なし | 実行者のキャラ属性に上書き |
| `Air` (1<<13) | エア | なし | 変化なし | 変化なし |
| `Memento` (1<<14) | メメント | 実行者のデフォルト精神属性で計算 | デフォルト精神属性に戻る | Noneならデフォルト精神属性が入る |
...
スキルの詳細（SkillPower計算、SkillType、SkillSpecialFlag）は `doc/スキル/スキル計算仕様書.md` を参照。
...
命中/回避の計算式は `doc/スキル/スキル計算仕様書.md` 8節を参照。
