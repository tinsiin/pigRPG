...
中央オブジェクトPresenter
- [x] 表示モード: HardBlock/SoftBlock/AutoTrigger（CenterTriggerMode enum）
...
- [x] 複数門 + order による優先度
- [x] 段階的解除（前門解除後に次門有効）
- [x] ループ歩数とリセット（resetOnSkip）
- [x] repeatable/cooldown
...
- [x] Edge fallback: ExitCandidate が空の場合 Edge を使用

分岐仕様
- [x] ExitCandidate/Edge による分岐表現
- [x] EdgeSO.conditions: エッジの条件
...
- Assets/Script/Walk/NodeSO.cs: exitSelectionMode/centerTriggerMode/encounterRateMultiplier追加
- Assets/Script/Walk/ExitCandidate.cs: conditions/weight追加
- Assets/Script/Walk/EdgeSO.cs: conditions追加
- Assets/Script/Walk/FlowGraphSO.cs: GetEdgesFrom()追加
- Assets/Script/Walk/AreaController.cs: ExitResolver統合/AutoTrigger対応
- Assets/Script/Walk/WalkProgressData.cs: 新規状態の保存/復元
- Assets/Script/Walk/Presentation/CentralObjectPresenter.cs: SFX対応
...
- GraphView エディタ: ノード/エッジの可視化編集UI
