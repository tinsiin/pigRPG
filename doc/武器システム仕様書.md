...
`BaseStates.TenDayValues(bool IsSkillEffect)` で制御:

- `IsSkillEffect=true` かつ `NowUseSkill != null` の場合:
  - スキルの `IsBlade/IsMagic/IsTLOA` フラグに応じたボーナスが加算
- `IsSkillEffect=false` の場合:
  - NormalData のみ（スキル特判ボーナスなし）
- **戦闘終了時**: `NowUseSkill = null` に設定（`BaseStates.CallBack.cs:196`）
  - これにより戦闘後はスキル特判ボーナスが外れ、NormalDataのみが適用される
...
| スキル特判別の十日能力ボーナス | `BaseStates.StatesNumber.cs:682-706` | Blade/Magic/TLOAボーナス |
...
[BaseStates]                  ┌─────────────┐
  ...
  │
  ├── TenDayValues(IsSkillEffect)
  │     ├── _baseTenDayValues（素の十日能力）
  │     └── + WeaponBonus（NormalData + スキル特判に応じたData）
  │           ▼
