...
| 要素 | 実装状況 | 備考 |
|---|---|---|
| 被害記録 | ✅実装済み | DamageRecord: 攻撃者・スキル・ダメージ・HIT結果・死亡フラグ。カウンター頻度→トラウマ率へ蓄積（4.7参照） |
| 行動記録 | ✅実装済み | ActionRecord: スキル・ターン・逃走フラグ。SkillUseCount/RecentActionsで参照可 |
| カウンター記録 | ✅実装済み | CounterRecord: カウンター実行者・使用中スキル・ターン |
| 死亡記録 | ✅実装済み | DeathRecord: 味方の死亡通知含む。AllyDeathCountで参照可 |
| トラウマ率 | ✅実装済み | 被害記録由来。カウンター頻度・HP急減・味方死亡で蓄積。IsTraumaAvoided/FilterByTraumaで回避判定（4.7参照） |
