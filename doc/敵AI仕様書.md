...
**判断ロジック自体の汎用部品化の価値は薄い。** 「いつ溜めるか」「いつ撃つか」はキャラの戦略そのものであり、派生AIのPlan()内で手書きする領域。
...
**判断ロジック自体の汎用部品化の価値は薄い。** ストック同様、「いつ溜め技を選ぶか」「カウント中にどう動くか」はキャラ固有の戦略であり、派生AIのPlan()内で手書きする領域。
...
現状、逃走は`AIDecision.IsEscape = true`でPlanから設定可能だが、「いつ逃げるか」を判断する思考部品がない。

**想定レベル:**
- **基本**: 単純な逃走確率（インスペクタ設定）
- **上級**: 逃走コールバック、HP減少率・被害履歴に基づく逃走判断
- **高度**: トラウマ率（割り込みカウンター頻度、HP急減）に応じた即逃走確率
...
ベストダメージ分析（セクション4.1）は**命中回避を計算しない、純粋な最大ダメージの思考**として設計されている。前のめり等の命中回避に影響する要素はベストダメージ分析の考慮外で、別途「命中回避シミュレート部品」で扱う想定。

**ベストダメージ分析から除外された要素と理由:**
- **前のめり**: 実装上もATK/DEF/ダメージ計算には一切影響せず、影響は命中回避系のみ（回避率半減、後衛命中低下、爆破型かすり化）。加えてベストダメージ分析はかなり基本的な部品であり、前のめりのようなやや特殊な要素を混ぜるべきではない。仮にダメージに影響があったとしても、基礎的なダメージ分析に混ぜず命中回避部品で扱う方針
- **敵のパッシブ**: 複雑すぎるため部品には組み込まない（メモ時点で除外判断済み）

**必要な部品案:**
- 命中率・回避率のシミュレート関数
- 前のめり（自分・敵）の影響計算
- 期待ダメージ（ダメージ×命中率）としてベストダメージ分析と統合する仕組み
...
| 敵のリカバリーターン | 未実装 | |
| 敵の前のめり | 未実装 | 命中回避系の部品で考慮予定 |
| 敵の物理属性 | 未実装 | |

#### 自分・味方の情報
| 要素 | 実装状況 | 備考 |
|---|---|---|
| 自HP / 自精神HP | 未実装（思考要素として） | 逃走・回復判断に必要 |
| 自分のパッシブ、味方のパッシブ | 未実装 | |
| 自分の人間状況 | 未実装 | |
| 味方、自分のリカバリーターン | 未実装 | |
| 味方前のめり連携 | 未実装 | |
