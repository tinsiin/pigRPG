...
本書は、既存の個別仕様書（`doc/スキル/スキル計算仕様書.md`, `doc/戦闘規格・狙い流れ仕様書.md`）ではカバーされていない戦闘メカニクスをまとめたもの。ダメージ計算式やATK/DEF/EYE/AGI係数、戦闘規格・AimStyleの排他係数については既存仕様書を参照。

関連仕様書:
- `doc/スキル/スキル計算仕様書.md` — ダメージ計算式、SkillType/SkillSpecialFlag、命中/回避、かすり率、ダメージ補正チェーン
- `doc/戦闘規格・狙い流れ仕様書.md` — BattleProtocol/AimStyle、排他ATK/DEF係数、防御切替、0.7クランプ、適応ラグ
- `doc/戦闘情報伝達/HP変動パス仕様書.md` — 戦闘中の全HP・精神HP変動パスの一覧と情報伝達層の対応
- `doc/戦闘情報伝達/フロー数字仕様書.md` — 第3層フロー数字の表示仕様
...
このボーナスは `SkillHitCalc` 内で命中率に加算される（`doc/スキル/スキル計算仕様書.md` 8.1節参照）。
...
レイザーダメージ発生時の数字ポップアップ表示は未実装。`doc/戦闘情報伝達/フロー数字仕様書.md` にカテゴリスタブあり。

---

## 戦闘終了の分岐

戦闘は以下の3つの結果（`BattleOutcome`）のいずれかで終了する。

実装: `BattleFlow.cs:142` `DialogEndAct()`, `BattleOutcomeTracker.cs`, `BattleOutcome.cs`

### 終了条件と判定フラグ

| 結果 | 判定フラグ | 終了メッセージ | 備考 |
|---|---|---|---|
| **Victory（勝利）** | `Wipeout && ActerFaction == Enemy` | 「勝ち抜いた」 | 敵側が全滅した |
| **Victory（勝利）** | `EnemyGroupEmpty` | 「敵はいなくなった」 | 敵が逃走等で全員離脱した |
| **Defeat（敗北）** | `Wipeout && ActerFaction == Ally` | 「死んだ」 | 味方側が全滅した |
| **Escape（逃走）** | `AlliesRunOut` | 「我々は逃げた」 | 味方全員が逃走選択で離脱 |

- `Wipeout`: どちらかの陣営が全滅したフラグ。`ActerFaction` が全滅した側の陣営を示す
- `EnemyGroupEmpty`: 敵グループが空になったフラグ（逃走・連鎖逃走で全員いなくなった場合）
- `AlliesRunOut`: 味方が逃走して離脱したフラグ

### 終了時のコールバック順序

`BattleFlow.DialogEndAct()` → `BattleManager.OnBattleEnd()` の順で処理が進む。

**DialogEndAct（結果確定）:**
1. 終了メッセージの表示
2. `BattleOutcomeTracker` に結果を通知（`OnPlayersWin` / `OnPlayersLost` / `OnPlayersRunOut`）
3. 敗北時: `EnemyGroup.EnemyiesOnWin()`（敵側の勝利コールバック）
4. 逃走時: `EnemyGroup.EnemiesOnAllyRunOut()`（味方逃走時の敵側コールバック）
5. `BattleEnded` イベント発火 → `OnBattleEnd` コールバック呼び出し

**OnBattleEnd（クリーンアップ）:**
1. 全キャラの特別補正リセット
2. 友情コンビ登録判定（**全結果で判定、結果倍率あり**。詳細は `doc/友情コンビ登録システム仕様書.md`）
3. 敵の復活準備（`RecovelyStart`）
4. 敵の終了時スキルAI
5. 全キャラの `OnBattleEndNoArgument` コールバック
6. UI復帰処理

### 逃走の詳細

逃走には**味方逃走**と**敵逃走**の2種類がある。

実装: `EscapeHandler.cs`

**味方逃走（`EscapeACT`）:**
- 味方が「逃げる」を選択 → エリアの逃走率（`EscapeRate`）で成否判定
- 成功時: `AlliesRunOut = true` → 戦闘終了（`Escape`）
- 失敗時: ターン続行

**敵逃走（`EscapeACT` 内の敵分岐）:**
- 敵AIが逃走を選択 → 逃走率判定
- 成功時: `EscapeAndRemove()` でグループから離脱
- 全員離脱なら `EnemyGroupEmpty = true` → 戦闘終了（`Victory`）

**連鎖逃走:**
- 敵が1体逃げた後、残りの敵に対して相性値≥77で無条件連鎖、それ以外は精神属性ごとの確率判定で連鎖
- 連鎖判定・除去は逃走と同一ターンで即時解決される（1ターン内で完結）
- 連鎖逃走者が2人以上の場合、**グループ逃走**として友情コンビ登録判定（×2.0）が発火する（`doc/友情コンビ登録システム仕様書.md` §3.3参照）

### 結果ごとの後続イベント

`EncounterSO` に結果ごとのイベントキューが設定可能。

| 結果 | イベント参照 |
|---|---|
| Victory | `EncounterSO.OnWinEvents` |
| Defeat | `EncounterSO.OnLoseEvents` |
| Escape | `EncounterSO.OnEscapeEvents` |

実装: `AreaController.cs:893` `TriggerOutcomeEvent()`, `NovelDialogueStep.cs:150`

---

## 割り込みカウンター

連続攻撃の2回目以降のHITに対して、被害者が攻撃を中断し反撃する仕組み。

実装: `BaseStates.BattleEvent.cs:890` `TryInterruptCounter()`, `BaseStates.ReactionSkill.cs:796`

### 判定条件

連続攻撃の2回目以降（`NowConsecutiveATKFromTheSecondTimeOnward()`）かつ、かすり以上のHIT時に判定が走る。**ただしFreezeConsecutive（ターンまたぎ連続攻撃）は判定をスキップする。** Freeze連続攻撃中は割り込みカウンターされない。

```
攻撃がHIT（かすり以上）
  → 連続攻撃の2回目以降か？
    → No → カウンター判定なし
    → Yes → FreezeConsecutiveか？
      → Yes → カウンター判定スキップ（Freeze保護）
      → No → TryInterruptCounter() 実行
```

### 発動確率（二段判定）

TryInterruptCounter内で以下の二段判定が行われる:

**前提条件:** `IsInterruptCounterActive == true` かつ `NowPower >= PowerLevel.Medium`

**第1段階:**
```
発動確率 = 攻撃スキルのDEFATK / 3 + max(0, 自Vond - 敵Vond) * 0.01
```
DEFATKが高い（防御無視率が高い）スキルほどカウンターされやすい。

**第2段階:**
```
counterValue = (自Vond + 自PersonaDivergence / (自TentVoid - ExplosionVoid)) * 0.9
attackerValue = max(敵Sort - 敵Rain/3, 0) + 敵ColdHeartedCalm
確率 = counterValue / (counterValue + attackerValue) * 0.5
```

両方通過して初めて割り込みカウンターが発動する。

### 発動時の処理

1. **攻撃者の連続攻撃を無効化**: `DeleteConsecutiveATK()` + `IsActiveCancelInSkillACT = true`
2. **カウンターパッシブ付与**: ID=1のパッシブをバッファに生成。攻撃者も同パッシブを持っていればパワー+1で上書き
3. **反撃予約**（生命タイプのみ）: `manager.Acts.Add()`で次ターンの行動を予約

### 反撃時のスキル制約

カウンター発動者が**FreezeConsecutive中**または**発動カウント（トリガー）中**の場合、現在使用中のスキルで反撃するわけにいかないため、**武器スキル（`NowUseWeapon.WeaponSkill`）に差し替え**て反撃する。

```
カウンター発動者の状態チェック:
  → Freeze連続攻撃の途中 or トリガーカウント中
    → Yes → NowUseSkill を武器スキルに差し替え、isfreeze=true で予約
    → No  → 現在のスキルでそのまま反撃
```

| 発動者の状態 | 反撃スキル | 理由 |
|---|---|---|
| 通常（何もしていない） | 現在のスキル | 制約なし |
| FreezeConsecutive中 | 武器スキル | Freeze中のスキルは多ターンに渡る前提であり、割り込み反撃には使えない |
| トリガーカウント中 | 武器スキル | 溜め中のスキルはまだ発動準備段階であり、反撃には使えない |

### 反撃のDEFATK継承

食らった攻撃のDEFATKが反撃スキル自身のDEFATKより大きい場合、食らった攻撃のDEFATKを継承する。敵の強い攻撃をカウンターしたときほど、反撃も強い防御無視率になる。反撃スキルのDEFATKの方が高ければそのまま使う。
