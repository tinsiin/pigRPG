...
> **注**: SkillTypeは「何をするか（What）」を表す軸。
> これとは別に `EffectResolutionMode` が「どう処理するか（How）」を表す直交軸として存在する。
> Packagedモード（旧Manual1）では単一命中判定 → ManualSkillEffectで原子的に効果を処理する。
...
### 8.5 ミニマムヒットチャンス（ケレン行動パーセント）

通常の命中/回避判定とは**独立した別レイヤー**の命中機会。ケレン行動パーセントの確率で発動し、発動すれば通常判定の結果に関係なく**かすりかクリティカルのどちらかが確定で発生**する。

「ケレン行動」とは、戦闘中にわざと変な動きをする（剣筋をずらす、迫る攻撃にすれ違おうとする等）こと。リスクと引き換えに通常ではありえない結果を起こす行動を指す。

#### 8.5.1 ケレン行動パーセントの定義

攻撃時と防御時で独立した値を持つ。デフォルトは `4.4%`。

| 値域 | 意味 | フレーバー |
|---|---|---|
| < 4.4% | 引き締め | 変な行動を抑え、堅実に戦う |
| = 4.4% | 偶然 | たまたま手元が狂ったり、当たり所が良かったりする |
| > 4.4% | 意図的 | わざとケレン味のある行動を取り、クリティカル/かすりを誘発する |

- **攻撃時ケレン行動パーセント**: クリティカルを狙って変な剣筋を出す。ただしかすり（ダメージ激減）のリスクも伴う
- **防御時ケレン行動パーセント**: かすりを狙って迫る攻撃をギリギリで避けようとする。ただしクリティカルを食らうリスクも伴う

> 定数: `CommonCalc.KerenACTRateDefault = 4.4f`

#### 8.5.2 パッシブによる上書き

デフォルト4.4%はパッシブで上書きされる。複数パッシブがある場合、**4.4%以外の値の中からランダムで1つ選ばれる**。全てが4.4%ならデフォルト値がそのまま返る。

```
rates = 全パッシブの攻撃時(or防御時)ケレン行動パーセント
rates = ratesから4.4を除外
ratesが空 → 4.4を返す
ratesが1つ以上 → ランダムで1つ選んで返す
```

> 実装: `BaseStates.HasPassives.cs:292-311` `PassivesAttackACTKerenACTRate()` / `PassivesDefenceACTKerenACTRate()`

#### 8.5.3 発生確率の計算（CalcMinimumHitChancePer）

攻撃側と防御側のケレン行動パーセントから、最終的なミニマムヒットチャンス発生確率を算出する。

**定数:**

| 定数 | 値 | 意味 |
|---|---|---|
| `synergy_threshold` | 7.0% | 高め合いボーナスの発生しきい値 |
| `synergy_bonus_per` | 0.2 | 高め合いボーナスの係数 |
| `KereKereModifier_per` | 0.09 | ケレケレ補正の係数 |

**計算手順:**

```
入力:
  Patk = 攻撃者の攻撃時ケレン行動パーセント
  Pdef = 防御者の防御時ケレン行動パーセント

① 基本確率
  Pfinal = max(Patk, Pdef)
  → 変な状況に持ち込もうとする側が一人でもいれば、その確率が使われる

② 高め合いボーナス（条件付き加算）
  if Patk > 7.0 AND Pdef > 7.0:
    excessAtk = Patk - 4.4
    excessDef = Pdef - 4.4
    bonus = (excessAtk + excessDef) × 0.2
    Pfinal += bonus
  → 両者がともに意図的にケレン行動を取ろうとしている場合のみ発動

③ ケレケレ補正（高め合い計算後に加算）
  大きい方のケレン行動パーセントが4.4を超えている場合、
  その持ち主の十日能力「ケレケレ」× 0.09 を加算
  （同値の場合は攻撃者側が優先）
  → ケレケレだけでは高め合いボーナスの発動条件を満たせない

return Pfinal
```

**魔法スキルの例外:** 魔法スキルの場合、上記計算結果にかかわらず発生確率は `2%` に固定される。

> 実装: `BaseStates.ReactionSkill.cs:191-234` `CalcMinimumHitChancePer()`

#### 8.5.4 発動時のかすり/クリティカル抽選

ケレン行動が発生確率を通過した場合、以下の二段構造で結果を決定する。
...
1/3のコイントスにより、ステータス差が大きくても**完全な確定にはならない**。弱い側にも常にチャンスが残る。

#### 8.5.5 通常命中判定との合流

ミニマムヒットチャンスの結果は、通常の命中判定と `OR` 条件で合流する。

```
if (通常命中判定が成功) OR (ケレン行動が発動した):
  攻撃が通る
  → ケレン行動発動時はCritical/Grazeが結果になる
  → ケレン行動なし＋通常命中なら、通常のHit判定へ進む
else:
  CompleteEvade（完全回避）
```

つまり：
- 通常なら完全回避される攻撃でも、ケレン行動が発動すればかすりかクリティカルとして当たる（**最低限の命中保証** = ミニマムヒットチャンスの名前の由来）
- 通常ならHitする攻撃でも、ケレン行動が発動すればCritical/Grazeに化ける

#### 8.5.6 計算例

| 攻撃側 | 防御側 | 基本確率 | 高め合い | 最終確率 | 解説 |
|---|---|---|---|---|---|
| 4.4% | 4.4% | 4.4% | なし | 4.4% + ケレケレ補正 | デフォルト同士。偶然のみ |
| 4.4% | 16% | 16% | なし（4.4 < 7） | 16% + ケレケレ補正 | 片方だけ意図的。大きい方がそのまま |
| 10% | 16% | 16% | (5.6+11.6)×0.2 = 3.44% | 19.44% + ケレケレ補正 | 両方意図的。高め合い発動 |
| 3% | 2% | 3% | なし | 3% | 両方引き締め。大きい方のみ |
| 7% | 8% | 8% | (2.6+3.6)×0.2 = 1.24% | 9.24% + ケレケレ補正 | しきい値ギリギリで高め合い |

> 実装: `BaseStates.ReactionSkill.cs:61-180` `IsReactHIT()`
...
AttackChara() 開始
  ...
  └─ 対象ごとに ReactionSkillOnBattle()
       ...
       │    ├─ ミニマムヒットチャンス: CalcMinimumHitChancePer() → ケレン行動発生確率
       │    │    └─ 発動時: 1/3コイントス or 2/3ステ比較 → Critical/Graze
       │    └─ 通常判定: SkillHitPer + supremacy vs Random(100)
       │    └─ 合流: 通常命中 OR ケレン発動 → 攻撃通過
       │
       ├─ 攻撃系:
...
| `BaseStates.ReactionSkill.cs:61-180` | IsReactHIT（命中判定 + ミニマムヒットチャンス合流） |
| `BaseStates.ReactionSkill.cs:191-234` | CalcMinimumHitChancePer（ケレン行動発生確率） |
| `BaseStates.HasPassives.cs:292-311` | パッシブ由来ケレン行動パーセント取得 |
