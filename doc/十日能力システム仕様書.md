...
> 関連仕様書: `doc/スキル/スキル計算仕様書.md`, `doc/精神分散防御仕様書.md`, `doc/十日能力・四大ステ成長レンジ想定.md`
...
> 実装: `BaseStates.Attack.cs`, `BaseStates.ReactionSkill.cs`, `BaseStates.StatesNumber.cs`, `Battle/Growth/WinGrowthStrategy.cs`

### 5.1 素振り/HIT の分離

スキル実行時の十日能力成長は **素振り**（スキル実行時に1回）と **HIT**（当たった相手ごとに1回）で**別々の十日能力値を参照**する。

```
SkillLevelData:
  TenDayValuesSwing   ← 素振り時に成長する十日能力
  TenDayValuesHit     ← HIT時に成長する十日能力
```

> 実装: `BaseSkill.SkillLevel.cs`（フィールド定義）, `BaseSkill.TenDays.cs`（取得メソッド）

#### 素振り

スキル実行時に **1回だけ** 発生。命中に関係なく成長する。

```
成長量 = グラデーション係数 × TenDayValuesSwing の各値 × 素振り倍率 × 自信ブースト
```

| スキルタイプ | 素振り倍率 |
|------------|----------|
| 攻撃スキル（`SkillType.Attack`） | 0.3 |
| 非攻撃スキル | 0.1 |

> 実装: `BaseStates.Attack.cs:22-28`

#### HIT

各被害者への命中判定成功時に発生。**当たった人数分だけ実行**される。

```
成長量 = グラデーション係数 × TenDayValuesHit の各値 × HIT倍率 × 相手比率 × 自信ブースト
```
...
相手比率は被害者の十日能力総量との比率で算出。格上相手ほど成長が大きい。

HIT分の成長は `TenDayGrowthListByHIT` にリスト記録され、全対象処理後にまとめて適用される。
これは同一攻撃内で途中の成長がダメージに影響するのを防ぐため。

TLOAスキルの場合、HIT時はゆりかごシステムで補正されたスキルレベルの `TenDayValuesHit` が使用される。

> 実装: `BaseStates.ReactionSkill.cs:982-995`, `BaseStates.Attack.cs:93-97`

#### 合算値の用途

成長以外の全用途（乖離判定、使いこなし度、敵スキル統計、TenDayValuesSum 等）では `TenDayValues()` メソッドが **Swing + Hit の合算**を返す。

```csharp
// BaseSkill.TenDays.cs
public TenDayAbilityDictionary TenDayValues() => TenDayValuesSwing() + TenDayValuesHit();
```

> 関連: `doc/スキル/スキルレベル仕様書.md` §3（フィールド定義）, §4（無限成長）
...
> 実装: `BaseStates.StatesNumber.cs:1020-1099`

戦闘勝利時、戦闘中に蓄積された成長量（`battleGain`）に対して **勝利倍率** を乗算し、追加成長を付与する仕組み。

#### battleGain のライフサイクル

```
戦闘開始 → battleGain = new()     // CallBack.cs:183
成長発生 → battleGain[ability] += 成長量  // GrowTenDayAbilityBySkill内
勝利時   → VictoryBoost(ratio)   // 勝利倍率を適用
戦闘終了 → battleGain.Clear()    // CallBack.cs:205
```

`battleGain` は戦闘中に素振り・HIT で蓄積された十日能力成長量の累計。勝利時にこの累計に対して倍率がかかる。

#### 強さ比率（ratio）

味方グループと敵グループの強さ比較で算出される。値が大きいほど格上の敵に勝ったことを意味し、倍率が高くなる。

#### 勝利倍率テーブル（16段階）

| ratio上限 | 倍率 | | ratio上限 | 倍率 |
|----------|------|---|----------|------|
| 0.6 | 2.6 | | 2.4 | 18 |
| 0.7 | 5.0 | | 2.6 | 20 |
| 0.8 | 6.6 | | 2.9 | 19 |
...
テーブルは上から順にチェックし、`ratio <= maxRatio` を最初に満たす行の倍率を返す。

**オーバーフロー**: ratio > 4.8 の場合 → `max(1.0, ratio - 7.0)`。超格上（ratio > 8.0）で再度大きくなるが、7.0付近は倍率1.0に沈む。
...
foreach (ability in battleGain.Keys)
{
    float totalGained = battleGain[ability];
    float extra = totalGained * (multiplier - 1);
    BaseTenDayValues[ability] += extra;       // 追加分を直接加算
    battleGain[ability] = totalGained * multiplier;  // 記録も更新
}
```

- `multiplier - 1` 分が追加成長（元の成長は既に適用済み）
- **敗北時**: 敵側に勝利ブーストが適用される（敵も成長する）
- 勝利側の **全メンバー**（生存・死亡問わず）に適用される

#### 設計意図

- 格上撃破の達成感を数値的に裏付ける
- ratio 2.9 で倍率が一度 19 に下がる（2.6 の 20 より低い）のは意図的な非線形設計
- 基礎上昇値（スキルの十日能力値）の適正レンジは **0.002〜0.004** を想定。勝利ブーストがかかることで実効的な成長量になる
