...
| **③ 威力・命中** | | | | |
| | SkillPower | float | 0 | |
| | TenDayValuesSwing | TenDayAbilityDictionary | null | 素振り時に成長する十日能力 |
| | TenDayValuesHit | TenDayAbilityDictionary | null | HIT時に成長する十日能力 |
...
| **⑧ ビジュアルエフェクト** | | | | |
| | CasterEffectName | string | "" | `[EffectName("icon")]` 術者に再生 |
| | TargetEffectName | string | "" | `[EffectName("icon")]` 各対象に再生 |
| | FieldEffectName | string | "" | `[EffectName("field")]` フィールド全体に再生 |
| **⑨ エフェクト・パッシブ** | | | | |
...
| `_infiniteSkillTenDaysSwingUnit` | float | BaseSkill | 素振り側の十日能力無限単位 |
| `_infiniteSkillTenDaysHitUnit` | float | BaseSkill | HIT側の十日能力無限単位 |
...
TenDayValuesSwing   = 最終のTenDayValuesSwing   + _infiniteSkillTenDaysSwingUnit   × 超過レベル数
TenDayValuesHit     = 最終のTenDayValuesHit     + _infiniteSkillTenDaysHitUnit     × 超過レベル数
```

無限単位を持つのは SkillPower と TenDayValuesSwing / TenDayValuesHit の3つだけ。他の値は無限範囲でも有限リスト最終の値がそのまま使われ続ける。
...
| ファイル | プロパティ数 | 内容 |
|---------|------------|------|
| `BaseSkill.Core.cs` | 20 | SkillName, SkillSpiritual, SkillPhysical, Impression, MotionFlavor, SpecialFlags, SkillType(バッファ合成), ConsecutiveType, ZoneTrait, RequiredNormalP, RequiredAttrP, RequiredRemainingHPPercent, Cantkill, SKillDidWaitCount, EvasionModifier, AttackModifier, AttackMentalHealPercent, CasterEffectName, TargetEffectName, FieldEffectName |
...
| `BaseSkill.TenDays.cs` | `TenDayValues()` = Swing+Hit合算, `TenDayValuesSwing()`, `TenDayValuesHit()` — 有限/無限分岐 |
