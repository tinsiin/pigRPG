...
- `Assets/Script/BattleAIBrains/BattleMemory.cs` — AI戦闘記憶（個体別、被害・行動・カウンター・かばい・死亡記録）
- `Assets/Script/BattleAIBrains/PostBattleBondCalculator.cs` — 戦闘後相性値変動計算
...
- `doc/終了済み/戦闘後AI相性値変動_設計書.md` — 戦闘後相性値変動の詳細設計
...
`HandleFreezeContinuation()`の処理順序:
1. **`ShouldAbortFreeze()`フック**: virtualメソッド（デフォルトfalse）。trueなら`DeleteConsecutiveATK()` → `DoNothing`で即終了。**ResumeFreezeSkillの前に呼ぶこと**（後だと状態不整合）
2. **`ResumeFreezeSkill()`**: 以下の内部処理を実行
...
`GetFreezeInfo()` → `FreezeInfo`構造体:
- `Skill`: 凍結中のスキル
- `IsFreeze`: 凍結中か
- `RangeWill`: 凍結中の範囲
- `CanOperate`: 操作可能（範囲/対象を変更できる）か
- `WillBeDeleted`: 次ターンで打ち切り予約があるか
...
TryInterruptCounterのロジックを確率論的に簡易近似。乱数を使わず、カウンターされる推定確率を返す。

**推定計算:**
1. **前提チェック**: `target.IsInterruptCounterActive == false` → 0。`target.NowPower < Medium` → 0
2. **Gate 1**: `skill.DEFATK / 3 + VondBonus`（VondBonus = ターゲットのVondが攻撃側より高い場合、差分 × 0.01）
3. **Gate 2**: 能力値比較。カウンター側 = `(Vond + PersonaDivergence/(TentVoid-10)) × 0.9`、攻撃側 = `max(Sort - Rain/3, 0) + ColdHeartedCalm`。比率 = counter / (counter + attacker)
4. **Gate 3**: 50%固定
5. **結果**: Gate1 × Gate2 × 0.5

**意図的な不完全性:** ExplosionVoidValueを10固定で近似。実際のTuning値とは異なる可能性がある。
...
`ShouldEscape()` (virtual): 基本確率(`_escapeChance`) + HP急減ボーナス(`HpDropRate × 0.2`)で逃走判定。派生AIでトラウマ率等を加味したoverride可能。
...
| `EvaluateDamage(target, skill)` | Analyzer内部用。精神レベルによる動的ポリシー調整 + useExpectedDamage時は期待ダメージ |
| `EvaluateDamageWithPolicy(target, skill, policy)` | ポリシー指定版（EvaluateDamageの内部ヘルパー） |
| `GetSpirituallyAwarePolicy(target)` | 精神レベルに応じてdamageType/spiritualModifierを動的調整 |

**Analyzerとの統合:** SingleBestDamageAnalyzer / MultiBestDamageAndTargetAnalyzer 内部で`EvaluateDamage()`を使用。精神レベル0なら従来通り（Inspector設定のみ）。精神レベル1+ではターゲットの精神HP状態に応じてdamageType/spiritualModifierが自動調整される。
...
2. トラウマフィルタ（`FilterByTrauma()`でカウンター経験のあるスキルを確率的に除外）
3. ダメージ分析（`AnalyzeBestDamage`でスキル＋ターゲット選定）
4. フォールバック（スキル1つの場合やダメージ分析不可時はランダム選択）

**Inspector設定で生まれる個性:**
- `SkillAnalysisPolicy`: ダメージ評価方法（物理/精神、DEF考慮、期待ダメージ、刻み・ブレ段階）
- `_canEscape` + `_escapeChance`: 逃走行動
- `_traumaPerCounter` + `_traumaHpDropWeight` + `_traumaPerAllyDeath`: トラウマ感度
- `_deliberationLevel` (0-3): 思慮推測レベル（パッシブ読み精度、カウンターリスク推定の可否）
- `_spiritualLevel` (0-3): 精神レベル（精神ダメージ判断の精度）
- `damageStep` + `variationStages`: 行動のバリエーション幅

### 4.7 Phase 2 記憶・学習システム

#### BattleMemory（`BattleMemory.cs`）

個体（NormalEnemy）ごとに保持するランタイム戦闘記憶。SOに持たせず個体に持たせることでSO共有汚染を回避。

- `BaseStates.AIMemory` (virtual) → `NormalEnemy`でoverride（`[NonSerialized]`、lazy初期化）
- 戦闘開始時に`Clear()`でリセット

**4つの記録:**

| 記録 | 構造体 | 記録タイミング | 主なフィールド |
|---|---|---|---|
| 被害 | `DamageRecord` | `DamageOnBattle()`内、HP減算後 | Attacker, Skill, Damage, MentalDamage, HitResult, Turn, ResultedInDeath |
| 行動 | `ActionRecord` | `CommitDecision()`後 + Targets補填 | Skill, Targets(List), Turn, WasEscape |
| カウンター | `CounterRecord` | `TryInterruptCounter()`内、カウンター成立時 | CounteredBy, SkillUsed, Turn |
| かばい | `ShieldRecord` | `TargetingService`でテラーズヒット発生時、前のめり者に即記録 | Attacker |
| 死亡 | `DeathRecord` | `DamageOnBattle()`内、Death()判定後 | Victim, Killer, KillerSkill, Turn, IsSelf, IsAlly, Affinity |

**死亡記録の通知:** 自分の死亡時、同一グループの生存味方全員に`IsAlly=true`の`DeathRecord`を通知（`NotifyAllyDeathToMemory()`）。加えて敵グループの生存メンバー全員に`IsAlly=false, IsSelf=false`の`DeathRecord`を通知（`CalcKillBonus`用）。

**ActionRecord.Targetsの補填:** AI行動決定時（`CommitDecision`）ではBM側のターゲット解決が未完了のため`Targets=null`で記録される。`SkillExecutor.SkillACT()`内で`SelectTargets()`完了後に`PatchLastActionTargets()`で全ターゲットをリストで補填する。これにより全体回復スキルでも全ターゲット分の回復ボンドが検出される。

**HPスナップショット:** `SkillActRun()`の各分岐末尾（思考完了後）に`SnapshotHPAfterDecision()`で`SnapshotHP(HP, MentalHP)`を記録。次ターンの`HpDropRate(currentHP)`で前ターンからのHP急減率を計算。

#### 記憶参照ユーティリティ（BattleAIBrain基底）

| プロパティ/関数 | 型 | 説明 |
|---|---|---|
| `Memory` | BattleMemory | 自キャラのAI戦闘記憶（なければnull） |
| `HpDropRate` | float | 前ターンからのHP急減率 (0.0~1.0) |
| `DamageThisTurn` | float | このターンに受けた合計ダメージ |
| `CounterCount` | int | カウンターされた総回数 |
| `CounterCountBySkill(skill)` | int | 指定スキルでカウンターされた回数 |
| `SkillUseCount(skill)` | int | 指定スキルの使用回数 |
| `AllyDeathCount` | int | 味方の死亡数（自分を含まない） |

#### トラウマシステム

感情・本能軸パラメータ。思慮レベル不問（どんなに愚鈍でもトラウマは蓄積する）。

**Inspector設定:**

| フィールド | 型 | デフォルト | 説明 |
|---|---|---|---|
| `_traumaPerCounter` | float (0~0.3) | 0.15 | カウンター1回あたりのトラウマ率上昇 |
| `_traumaHpDropWeight` | float (0~1) | 0.3 | HP急減時のトラウマ率ボーナス倍率 |
| `_traumaPerAllyDeath` | float (0~0.3) | 0.1 | 味方死亡1回あたりのトラウマ率上昇 |
| `_traumaCap` | float (0~1) | 0.8 | トラウマ率の上限 |

**トラウマ率の計算:**
```
TraumaRate = Clamp(counterCount × _traumaPerCounter
           + HpDropRate × _traumaHpDropWeight
           + AllyDeathTraumaWeight × _traumaPerAllyDeath, 0, _traumaCap)
```

**味方死亡の相性値重み付け（AllyDeathTraumaWeight）:**
...
相性値は`BattleGroup.CharaCompatibility`から取得し、`DeathRecord.Affinity`に記録時点で保存。

**IsTraumaAvoided(skill):** そのスキルでカウンターされた経験がある場合のみ判定対象。スキル固有トラウマ = `TraumaRate × (そのスキルでのカウンター回数 / 全カウンター回数)` で`Roll()`。

**FilterByTrauma(skills):** スキルリストから`IsTraumaAvoided()`で除外。全スキルが除外された場合は元のリストにフォールバック（行動不能を防止）。

#### ShouldEscape()の拡張

基本確率に加え、HP急減率が逃走確率を押し上げる:
```
effectiveChance = _escapeChance + HpDropRate × 0.2
```
...
### 6.2 戦闘後相性値変動（PostBattleBondCalculator）

`BattleManager.OnBattleEnd()`内で、PostBattle行動（6.3）の**前**に実行される。BattleMemoryの記録を振り返り、敵同士の相性値を変動させる。

**呼び出し:** `PostBattleBondCalculator.Apply(EnemyGroup)`

**変動トリガー（仮値）:**

| トリガー | 変動量 | 検出方法 | 状態 |
|---------|--------|---------|------|
| 味方から回復を受けた | +7 | healerのActionRecord（Target補填済み、Heal/MentalHeal/DeathHeal） | ✅実装済み |
| 敵を殺した | +4 | DeathRecord（!IsAlly && !IsSelf）→ グループ全ペアに加算 | ✅実装済み |
| 味方が逃走した | -15 | `BattleGroup.EscapedMembers` × 残留者の全ペア | ✅実装済み |
| 味方がかばってくれた（テラーズヒット） | +10 | ShieldRecord（TargetingServiceでテラーズヒット発生時に前のめり者のBattleMemoryに即記録） | ✅実装済み |

**反映先:**
- `BattleGroup.CharaCompatibility` — PostBattle行動用に即反映（0〜160クランプ）
- `NormalEnemy._bondDeltas` — 個体に永続化（次回エンカウント用）
- 逃走ボンドは `_bondDeltas` のみ更新（逃走者はOursから除去済みのためCharaCompatibility参照不可）

**次回エンカウント時の合算:**

`EncounterEnemySelector.BuildCompatibilityData()`で、CSV基礎値に`_bondDeltas`の双方向平均を加算:
```
最終相性値 = Clamp(CSV基礎値 + (A.GetBondDelta(B) + B.GetBondDelta(A)) / 2, 0, 160)
```

詳細は `doc/終了済み/戦闘後AI相性値変動_設計書.md` を参照。
...
Plan(decision):
  ...
  2. FilterByTrauma(availableSkills) → candidates（トラウマ回避フィルタ、全除外時はフォールバック）
  3. GetPotentialTargets()で攻撃対象列挙
  4. candidates >= 2 かつ targets > 0 なら AnalyzeBestDamage(candidates, targets) でベストスキル+ターゲット選定
  5. 完全単体選択スキルなら decision.TargetWill = DirectedWill.One
  6. フォールバック: candidates1つならそれを使用、複数ならランダム選択（完全単体選択スキルならTargetWill=Oneも設定）
```
...
| `_bondDeltas` | 永続（ランタイム） | 相性値変動の累積記録（`Dictionary<string, int>`、`_aiMemory`と同設計） |
| `RecordBondDelta(guid, delta)` | 戦闘終了時 | 相手GUIDに対する変動量を累積加算 |
| `GetBondDelta(guid)` | エンカウント時 | 相手GUIDに対する累積変動量を返す |
...
- **上級**: ✅ HP急減率（`HpDropRate`）が逃走確率にボーナス加算（`+HpDropRate×0.2`）→ セクション4.7参照
- **高度**: トラウマ率に応じた即逃走確率は派生AIでoverride可（基盤は整備済み、`TraumaRate`プロパティ利用可）
...
AIが判断材料として参照しうる要素の全体像。各要素について11.6の設計思想（情報取得/判断ロジック/操作の3層フレームワーク）に基づく実装方針を記載する。

**凡例:**
- **情報取得**: 既存のpublicアクセスで足りるか、新ユーティリティが必要か
- **判断ロジック**: ヘルパー化すべきか、Plan()手書きか
- **操作**: 既存CommitDecisionパイプラインで足りるか

---

#### 敵（攻撃対象）の情報

| 要素 | 実装状況 | 情報取得 | 判断ロジック | 操作 |
|---|---|---|---|---|
| 敵HP | ✅対応済み | SimulateDamageで使用 | AnalyzeBestDamage（ヘルパー） | 既存 |
| 敵精神HP | ✅対応済み | mentalDmgモード | EvaluateDamage + 精神レベル（ヘルパー） | 既存 |
| 敵群総HP / 総精神HP | 基盤済み | `GetPotentialTargets()`でリスト取得済み。HP/MentalHPはpublic。foreachで即合計可能→**新ユーティリティ不要** | 「全体HPが低い→範囲攻撃に切替」等→**キャラ固有、Plan()手書き** | 既存 |
| 敵精神属性（相性） | ✅対応済み | `spiritualModifier`オプション（SkillAnalysisPolicy） | 精神レベル1+で`GetSpirituallyAwarePolicy()`が自動調整（**ヘルパー済み**） | 既存 |
| 敵パッシブ | ✅基盤済み | `ReadTargetPassives(target)` / `CanSeePassive(target, id)` 実装済み（思慮レベル依存） | パッシブ内容に応じた判断（防御パッシブ持ちを避ける、IsCantACTを見てFreeze継続等）→**キャラ固有、Plan()手書き** | 既存 |
| 敵のパワー | 基盤済み | `target.NowPower` はpublic→**新ユーティリティ不要**。EstimateCounterRisk内部でも`NowPower >= Medium`判定に使用 | パワーレベルに応じた行動変更→**キャラ固有、Plan()手書き**（思慮レベル2+想定） | 既存 |
| 敵のリカバリーターン | 基盤済み | `target.RecoverTurn` はpublic→**新ユーティリティ不要** | 「回復中の敵を狙う/無視する」→**キャラ固有、Plan()手書き** | 既存 |
| 敵の前のめり | ✅対応済み | HitSimulatePolicy.targetIsVanguard（4.5参照） | SimulateHitRateが爆破型+前のめりの命中率を自動近似（**ヘルパー済み**） | 既存 |
| 敵の物理属性 | 基盤済み | `target`のプロパティでpublic→**新ユーティリティ不要** | 物理属性相性に応じたスキル選択→**キャラ固有、Plan()手書き** | 既存 |

#### 自分・味方の情報

| 要素 | 実装状況 | 情報取得 | 判断ロジック | 操作 |
|---|---|---|---|---|
| 自HP / 自精神HP | ✅対応済み | HpRatio, MentalHpRatio, IsLowHP（4.5参照） | ShouldEscape等で活用済み（**ヘルパー済み**） | 既存 |
| 自分のパッシブ | 基盤済み | `user.Passives` はpublic→**新ユーティリティ不要** | 自分のパッシブに応じた行動変更→**キャラ固有、Plan()手書き** | 既存 |
| 味方のパッシブ | 基盤済み | `EnumerateGroupAllies()` + `.Passives` で取得可能→**新ユーティリティ不要** | 味方のパッシブに応じた連携→**キャラ固有、Plan()手書き** | 既存 |
| 自分の人間状況 | 基盤済み | userのプロパティでpublic→**新ユーティリティ不要** | **キャラ固有、Plan()手書き** | 既存 |
| 味方・自分のリカバリーターン | 基盤済み | public→**新ユーティリティ不要** | **キャラ固有、Plan()手書き** | 既存 |
...
| 要素 | 実装状況 | 情報取得 | 判断ロジック | 操作 |
|---|---|---|---|---|
| 被害記録 | ✅実装済み | Memory, DamageThisTurn等（4.7参照） | トラウマ蓄積（**ヘルパー済み**）。戦略変更は手書き | — |
| 行動記録 | ✅実装済み | SkillUseCount, RecentActions等（4.7参照） | 手書き | — |
| カウンター記録 | ✅実装済み | CounterCount, CounterCountBySkill（4.7参照） | トラウマ蓄積（**ヘルパー済み**） | — |
| 死亡記録 | ✅実装済み | AllyDeathCount（4.7参照） | トラウマ蓄積（**ヘルパー済み**） | — |
| トラウマ率 | ✅実装済み | TraumaRate（4.7参照） | FilterByTrauma / IsTraumaAvoided（**ヘルパー済み**） | — |

#### スキル選択の追加考慮

| 要素 | 実装状況 | 情報取得 | 判断ロジック | 操作 |
|---|---|---|---|---|
| スキル使用率（重み付け選択） | 基盤済み | `availableSkills`リストは既存→**新ユーティリティ不要** | **Plan()手書き**（`Roll()`で確率分岐）。ヘルパー化が不適切な理由: SOが複数キャラで共有されるため、Inspector上でスキル名→重みのマッピングを持つのが困難。また`variationStages`が「ダメージ上位N個からの重み付き抽選」として類似機能を既に提供 | 既存 |
| スキル攻撃性質 | 部分的対応 | ZoneTraitは参照済み。PhysicalProperty等はBaseSkillプロパティでpublic→**新ユーティリティ不要** | **キャラ固有、Plan()手書き** | 既存 |
| スキル分散性質 | ✅暗黙対応済み | `skill.DistributionType` はpublic→**新ユーティリティ不要** | `useExpectedDamage=true`ならSimulateHitRateが爆破型+前のめりの命中率補正を自動近似するため、期待ダメージ評価で**暗黙的に考慮済み**（**ヘルパー済み**）。明示的な分散性質判断は手書き | 既存 |
| 防御無視率の高いスキル | ✅基盤済み | `skill.DEFATK` はpublic。`EstimateCounterRisk(target, skill)`が内部でDEFATK/3を使用→**新ユーティリティ不要** | **事後回避（実装済み）:** トラウマシステム。カウンター経験スキルを`FilterByTrauma`で回避（**ヘルパー済み**）。**事前回避:** カウンター前から`EstimateCounterRisk() > 閾値`でスキップ→**Plan()手書き**（思慮レベル2+想定。`if (DeliberationLevel >= 2 && EstimateCounterRisk(target, skill) > 0.3f) skip`のように記述） | 既存 |
...
Phase 1-3の基盤整備により、全思慮要素について**情報取得は既存のpublicアクセスまたは実装済みユーティリティで対応可能**。新たな情報取得ユーティリティが必要な要素はない。判断ロジックについては、ヘルパー化すべきもの（AnalyzeBestDamage, SimulateHitRate, トラウマフィルタ, 精神レベル自動調整等）は全て実装済み。残りは全てキャラ固有の戦略判断であり、Plan()手書きの領域。操作・コミット層は全要素が既存パイプラインで対応可能。

上記は主要な思慮要素の一覧だが、戦闘コードベース全体の網羅的調査で追加52要素が発掘されている。詳細は `doc/敵AI思慮要素_コードベース発掘レポート.md` を参照。カテゴリ: シミュレーション精度系(12), 敵追加情報(7), グループ追加情報(8), スキル追加属性(11), パッシブ詳細(10), 特殊メカニクス(4)。いずれも情報取得は既存publicアクセスで対応可能、新ヘルパー不要。

---

### 11.3 思慮推測のレベル ✅実装済み

`_deliberationLevel` (0-3) のInspectorフィールドとして実装。派生AIが`DeliberationLevel`プロパティで参照し、条件分岐に使用する。

| レベル | 可能になること | 実装 |
|---|---|---|
| 0 | 情報読みなし。基本のダメージ分析のみ | （デフォルト、既存のBasicTacticalAI相当） |
| 1 | HP・基本状態の判断 | HpRatio, MentalHpRatio, IsLowHP等 |
| 2 | パッシブ読み（不完全、70%認識率）+ カウンターリスク推定 | `ReadTargetPassives`, `CanSeePassive`, `EstimateCounterRisk` |
| 3 | 高精度パッシブ読み（90%認識率）+ ポイント推測 | 同上（精度向上） |

- `ReadTargetPassives(target)`: ターゲットのパッシブリストの**コピー**を返す（直接参照で戦闘システム破壊を防止）。思慮レベル0-1では空リスト
- `CanSeePassive(target, passiveId)`: 特定IDのパッシブを認識できるか判定（思慮レベル依存の確率判定）
- Lv3でも認識率100%にはしない（「ズルい」と感じさせない設計）
- Freeze中断判断（ShouldAbortFreeze）は派生AIがDeliberationLevelに基づいてoverrideする想定
...
`_spiritualLevel` (0-3) のInspectorフィールドとして実装。`EvaluateDamage`内部で自動的に`SkillAnalysisPolicy`を調整する。

| レベル | 可能になること | 実装上の変化 |
|---|---|---|
| 0 | 精神系の判断なし。物理ダメージのみ | `damageType`はInspector設定のまま |
| 1 | ターゲットの精神HPが物理HPより削れていれば精神ダメージを狙う | `damageType`を動的切り替え |
| 2 | さらに精神属性相性を考慮 | `spiritualModifier=true`を自動有効化 |
| 3 | 物理と精神の両方を評価し、高い方を採用 | 両damageTypeで`SimulateDamage`を実行して比較 |

- `GetSpirituallyAwarePolicy(target)`: 精神レベルに応じてpolicyを動的調整する内部メソッド
- `EvaluateDamageWithPolicy(target, skill, policy)`: ポリシー指定でダメージ評価する内部ヘルパー
- 思慮レベルが低くても精神レベルが高ければ、精神系の判断は鋭くなりうる（独立軸）

#### トラウマ率 ✅実装済み
- 被害記録由来の反応パラメータ。カウンター被弾・HP急減・味方死亡で蓄積（セクション4.7参照）
- Inspector設定で感度を個体ごとに調整可能（`_traumaPerCounter`, `_traumaHpDropWeight`, `_traumaPerAllyDeath`, `_traumaCap`）
- **実装済み影響:**
  - **スキル回避**: カウンターされた経験のあるスキルを確率的に使わなくなる（`IsTraumaAvoided` → `FilterByTrauma`）。BasicTacticalAIで統合済み
  - **逃走確率上昇**: HP急減率が`ShouldEscape()`の逃走確率にボーナス加算
- **将来拡張候補:**
  - 連続攻撃の回避判断（防御無視率ベース）: 防御無視率が高い攻撃ほど割り込みカウンターされやすい（`TryInterruptCounter`で`DEFATK/3`が発動確率に直結）。代表防御無視率しきい値による精密な回避
  - 再戦時の即逃走: 前回戦闘のトラウマ記録を跨いで持ち越し
...
**具体例: 実装済み要素での適用実績 + 未着手要素の方針**

| 要素 | 情報取得 | 判断ロジック | 操作 | 状態 |
|---|---|---|---|---|
| 逃走思考（11.1.4） | HpRatio, HpDropRate（✅ユーティリティ済み） | ShouldEscape()（✅ヘルパー済み。基本確率+HP急減ボーナス） | `IsEscape=true`で既存パイプライン | ✅実装済み |
| 被害記録（11.2） | Memory, DamageThisTurn等（✅ユーティリティ済み） | トラウマ蓄積（✅ヘルパー済み）。戦略変更→手書き | — | ✅実装済み |
| 命中回避シミュレート（11.1.6） | SimulateHitRate（✅ヘルパー済み） | EvaluateDamage + useExpectedDamage（✅ヘルパー済み） | — | ✅実装済み |
| Freeze中断（11.1.5） | GetFreezeInfo / EstimateCounterRisk（✅ユーティリティ済み） | ShouldAbortFreeze()（✅virtualフック済み）。具体判断→手書き | `DeleteConsecutiveATK()`で既存機構 | ✅基盤済み |
| パッシブ読み（11.2） | ReadTargetPassives / CanSeePassive（✅ユーティリティ済み） | パッシブに応じた行動変更→**手書き**（キャラ固有） | 既存パイプライン | ✅基盤済み |
| スキル使用率（11.2） | availableSkillsリスト（既存） | **手書き**（SO共有でスキル→重みマッピング困難。variationStagesが類似機能提供） | 既存パイプライン | 方針確定 |
| 防御無視率回避（11.2） | skill.DEFATK + EstimateCounterRisk（既存） | 事後: FilterByTrauma（✅済み）。事前: **手書き**（思慮Lv2+） | 既存パイプライン | ✅基盤済み |
| ポイント温存（11.2） | CanCastSkill + スキルコスト（既存） | **手書き**（温存基準がキャラ固有、引数発散パターン） | 既存パイプライン | 方針確定 |
...
| 2026-03-08 | 戦闘後相性値変動実装: 6.2にPostBattleBondCalculator追加、9章にNormalEnemy._bondDeltas追加、関連ファイル/仕様書に追記 |
