...
```
【EventStep統合後の規格（2026-01-26実装）】

EventDefinitionSO
├── steps: IEventStep[]
│   ├── MessageStep（メッセージ＋選択肢）
│   ├── NovelDialogueStep（ノベルパート）
│   ├── BattleStep（戦闘起動）← リアクションで使用可能
...
リアクションシステム
├── 戦闘 → EncounterSO + BattleRunner（現状）
└── （将来）EventDefinitionSO経由で任意のStepを実行可能
```

**利点:**
- EventStep統合により、全イベントが同じ規格で扱える
- `ReactionType` enumに追加するだけで拡張可能
- 将来は `ReactionType.Event` で EventDefinitionSO を直接参照することも可能
