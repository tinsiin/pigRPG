...
| INovelEventUI | `UpdateCentralObjectSprite()`, `GetCurrentCentralObjectSprite()` | ❌ 未実装 |
| NovelPartEventUI | CentralObjectPresenter統合 + 両メソッド実装 | ❌ 未実装 |
| NovelPartDialogueRunner | 中央オブジェクト更新処理 + スナップショット取得 | ❌ 未実装 |
| DialogueStateSnapshot | `CentralObjectSprite` フィールド + Clone修正 | ❌ 未実装 |
| DialogueContext | ~~CentralObjectPresenter参照~~ 不要 | ✅ CentralObjectRTで十分 |
