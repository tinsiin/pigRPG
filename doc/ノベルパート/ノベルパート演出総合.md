...
テキストボックスの登場・退場アニメーションが発生するコンテキストの全体像。

#### 発生コンテキスト一覧

| コンテキスト | 登場演出 | 退場演出 | 備考 |
|---|---|---|---|
| **会話開始時** | 使われる | - | 初回ステップでSwitchTextBox内のFadeInが走る |
| **モード切替時** | 使われる（新テキストボックス） | 使われる（旧テキストボックス） | FadeOut → FadeIn の順 |
| **会話終了時** | - | **使われていない** | SetTabState(walk)でUI全体が消えることで暗黙的に消える |

**現状の問題**: 退場演出はモード切替時にしか使われておらず、会話終了時にはテキストボックスの退場演出が走らない。
立ち絵はExit()で明示的に退場するが、テキストボックスはUIレイヤーごと消える。

**レタッチでの改善方針**（詳細は [テキストボックス・立ち絵レタッチ仕様書](./テキストボックス・立ち絵レタッチ仕様書.md) 参照）:
- 会話終了時にも退場演出を入れる。これが退場演出の**基本形（フル演出）**になる
- モード切替時の退場は、基本形から装飾を削ぎ落とした**切替用アレンジ（テンポ重視版）**として定義する
- 例: ディノイドの基本形は「テキスト消去→枠線点滅→閉じる」、切替用は「点滅なしで即閉じる」

#### テキストボックス登場演出（FadeInTextBox）

現状は Dinoid / Portrait 共通。
...
- 3要素を同時にアニメーション:
  ...
  - 回転: 5° → 0°
  - 透明度: 0 → 1
- 時間: 0.3秒、イージング: OutBack

#### テキストボックス退場演出（FadeOutTextBox）

現状は Dinoid / Portrait 共通。

**フェードアウト:**
- alpha: 1 → 0
- 時間: 0.15秒、イージング: InQuad
- 退場後に位置と回転をリセット
...
テキストボックスの退場/登場は上記のライフサイクルで定義されたアニメーションが使われる。
...
立ち絵のトランジション（登場・退場・変更）が発生する全コンテキスト。

| # | コンテキスト | 何が起きるか | 使われるトランジション | 定義状況 |
|---|---|---|---|---|
| 1 | **会話開始時（初登場）** | 最初のステップで立ち絵が出現 | 登場トランジション（transitionTypeで指定: Rockman/Slide等） | 定義済み |
| 2 | **ステップ間: 表情変更** | 同キャラで表情だけ変わる | 指定されたtransitionTypeで新スプライトが登場 | 定義済み |
| 3 | **ステップ間: キャラ変更** | 同じ側（左/右）で別キャラに切り替わる | クロスフェード（自動検出、フェードアウト0.1秒+フェードイン0.1秒） | 定義済み |
| 4 | **ステップ間: キャラ消去** | PortraitStateがnullになる | Hide() — その場でフェードアウト（0.3秒） | 定義済み |
| 5 | **Dinoid→Portrait切替** | ディノイドから立ち絵モードへ | 登場トランジション（#1と同じ） | 定義済み |
| 6 | **Portrait→Dinoid切替** | 立ち絵からディノイドへ | Exit() — 左右にスライドアウト（±500px）。Runnerが自動実行 | 定義済み |
| 7 | **会話終了時** | 会話完了のクリーンアップ | Exit() — #6と同じ。finallyブロックで自動実行 | 定義済み |

**補足:**
- **#1と#5は同じトランジション**を使う。モード切替による登場でも通常の登場でも同じ。
- **#6と#7は同じExit()**を使う。テキストボックスのような「基本形と切替用アレンジ」の区別はまだない。
- **#3のクロスフェード**は登場トランジション（Rockman等）を使わない。CharacterIdの変化を自動検出して常にクロスフェード。詳細パラメータはPortraitPresenter Inspectorの`crossFadeOutDuration`/`crossFadeInDuration`で調整可能。
- **#2の表情変更**は毎回transitionTypeが走る。元メモの「最初に出た時にのみ派手なトランジション」という自動判定は未実装（設計書「未実装」セクションに記載）。
...
立ち絵とテキストボックスの退場はUIが**まだ見えている間**に実行される。
...
├── SetTabState(walk)                UIレイヤー非表示（テキストボックス含む全UIが消える）
│
│  ── NovelDialogueStep側 ──
...
| 要素 | 退場方法 | 状態 |
|------|---------|------|
| **立ち絵（Portraitモード時）** | Exit(): 左右スライドアウト（±500px） | 明示的な退場演出あり |
| **テキストボックス** | **退場演出なし** | SetTabState(walk)でUI全体が消えることで暗黙的に消える |
| **UI全体** | SetTabState(walk) | 立ち絵Exit()完了後に非表示 |

**現状の挙動**: 立ち絵だけはスライドアウトで丁寧に退場するが、テキストボックスは退場演出がなくUIレイヤーごとパッと消える。
Dinoidモード / Portraitモードでの終了時の処理の違いもない（同じfinallyブロック）。
