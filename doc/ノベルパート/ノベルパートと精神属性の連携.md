...
```csharp
// 歩行中
GameContextHub.Current.Players.Roster.GetAllyById(AllyId.Geino)
...
BaseStates（基盤）
  ├─ AllyClass（味方共通）
  │   ├─ StairStates（Geino）
  │   ├─ BassJackStates（Noramlia）
  │   └─ SateliteProcessStates（Sites）
  └─ NormalEnemy（敵共通）
```
...
**ノベルパートとゲームロジック（AllyClass）は完全に分離されている**

| 項目 | ノベルパート | ゲームロジック |
|------|------------|--------------|
| ID体系 | characterId（文字列） | AllyId（enum） |
| キャラ名 | speaker（任意文字列） | CharacterName |
| 管理方式 | PortraitDatabase（SO） | PlayersRoster |
| 精神属性 | **なし** | MyImpression |

**この分離により:**
- ノベルパートは単体でプロトタイピング可能
- ゲームに存在しないキャラも登場可能
- しかし、実際のパーティー状態（精神属性）を参照できない
...
**現在の精神属性表示（ディノイドモード）:**
```
NovelDialogueStep.GetProtagonist(): AllyId.Geino
    ↓
GameContext.Players.Roster.GetAllyById(AllyId.Geino)
    ↓
...
**characterId（PortraitDatabase用）と protagonist（AllyId）は独立**

- characterId: 立ち絵表示用（文字列、NPCも可）
- protagonist: 精神属性処理対象（AllyId、パーティーメンバーのみ）

これにより:
- NPCも立ち絵に登場可能（精神属性なし）
- 主人公（精神属性を持つキャラ）は別途明示的に指定
...
```csharp
// DialogueContext.cs または専用ヘルパーに追加
public static SpiritualProperty? GetProtagonistImpression(
    AllyId? protagonistId,
    IPlayersRoster roster)
{
    if (protagonistId == null || roster == null) return null;
    var ally = roster.GetAllyById(protagonistId.Value) as AllyClass;
...
| タスク | 詳細 |
|--------|------|
| NovelDialogueStep拡張 | `bool hasProtagonist` + `AllyId protagonist` 追加（シリアライズ対応） |
...
### Phase 4: 立ち絵モード可視化（将来）

| タスク | 詳細 |
|--------|------|
| 仕様決定 | 精神属性ごとの立ち絵 or オーバーレイ |
| PortraitPresenter拡張 | 精神属性に応じた表示 |
...
1. **Phase 1実装**: NovelDialogueStepにprotagonist追加
2. **Phase 2実装**: 選択肢UIで精神属性表示
3. **Phase 3実装**: ディノイドモードで精神属性表示
4. **動作確認**: テストダイアログで検証
