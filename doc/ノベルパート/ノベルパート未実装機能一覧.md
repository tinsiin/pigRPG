...
- [ ] **主人公設定を CharacterId 化**（新キャラクター追加システム対応）
...
| 主人公設定 | `hasProtagonist` + `AllyId protagonist`（シリアライズ対応）→ **CharacterId に変更予定** |
| 関連設計 | [新キャラクター追加システム設計.md](../新キャラクター追加システム設計.md) Phase G |

> **Note**: 立ち絵描き分けは不要と判断。精神属性は内面的概念であり表情で表現すべきものではない。PortraitDatabaseのexpression（表情）とは別軸。

> **TODO**: 現在 `AllyId`（固定3人enum）で主人公を指定しているが、新キャラクター追加システムに合わせて `CharacterId`（string）に変更が必要。
