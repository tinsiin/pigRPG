...
EventDefinitionSO（既存）
    └── steps: IEventStep[] [SerializeReference]
        └── NovelDialogueStep
                └── dialogue: DialogueSO [SerializeField]

DialogueSO（新規）  ← 会話データ（フィールド/イベント両用）
```

- DialogueSOは会話データを保持するSO
- NovelDialogueStep.allowBacktrackで「戻れるか」を制御
- EventDialogueSOは不要（allowBacktrackフラグで代替）
...
> **Note**: EventDialogueSOは実装しない。
> 「戻れるか」の制御はNovelDialogueStep.allowBacktrackフラグで行う。
> 詳細は[ノベルパート課題一覧.md](./ノベルパート課題一覧.md)の課題3を参照。
...
> **Note**: EventDialogueSOは実装しない。NovelDialogueStep.allowBacktrackで「戻れるか」を制御。
...
**対策**: DialogueSOは新規作成、既存SOは段階的に移行
