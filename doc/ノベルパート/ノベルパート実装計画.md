...
| タスク | 工数目安 | 依存 |
|--------|---------|------|
| IDialogueRunner定義（IBattleRunnerパターン） | 1h | - |
| DialogueContext/DialogueResult定義 | 1h | IDialogueRunner |
| DialogueStep構造体定義 | 2h | - |
| FieldDialogueSO作成 | 2h | DialogueStep |
| INovelEventUI定義（IEventUI継承） | 2h | - |
| NovelPartEventUI骨格 | 3h | INovelEventUI |
| GameContext.DialogueRunner追加 | 0.5h | IDialogueRunner |
...
| タスク | 工数目安 | 依存 |
|--------|---------|------|
| NovelPartDialogueRunner実装 | 6h | Phase 1-4 |
| ForcedEventTrigger定義 | 2h | - |
| ForcedEventState/StateManager実装 | 3h | - |
| NodeSO.ForcedEventTriggers追加 | 1h | ForcedEventTrigger |
| GameContext.ForcedEventStateManager追加 | 0.5h | StateManager |
| AreaController.CheckForcedEvents()追加 | 3h | 全て |
| WalkingSystemManager初期化修正 | 1h | 全て |
| 既存イベント（ゲート/遭遇）との共存テスト | 2h | 全て |
