...
歩行システム
    ↓
中央オブジェクト表示（NodeSO.centralVisual で見た目決定）
    ↓
ユーザーがアプローチ
    ↓
...
```csharp
// NodeSO.cs
[SerializeField] private CentralObjectVisual centralVisual;  // 見た目
[SerializeField] private EventDefinitionSO centralEvent;     // アプローチ時イベント

// CentralObjectVisual 構造体
...
3. **ノベルパート終了時の復元**: ノベルパート終了→ズームアウト後、元のスプライトに戻すかどうかは要検討
...
| B: 変更維持 | 会話で変更したスプライトを維持 | 会話の影響が残る | 歩行システム側の想定と不整合 |
| C: 明示的指定 | DialogueSOで復元するか選択 | 柔軟 | 設定が複雑 |

→ **方針A（自動復元）を推奨**。ノベルパート終了時に元のスプライトに戻す。永続的な変更が必要な場合はEffectで別途対応。
...
- [ ] CentralObjectPresenter.UpdateSprite() 追加
- [ ] CentralObjectPresenter.GetCurrentSprite() 追加
- [ ] DialogueStep.centralObjectSprite 追加

### Phase 2: UI層

- [ ] INovelEventUI.UpdateCentralObjectSprite() 追加
- [ ] NovelPartEventUI に CentralObjectPresenter 参照追加
- [ ] NovelPartEventUI.UpdateCentralObjectSprite() 実装

### Phase 3: Runner統合

- [ ] NovelPartDialogueRunner で中央オブジェクト更新処理追加
- [ ] DialogueStateSnapshot に CentralObjectSprite 追加
- [ ] 戻る機能での中央オブジェクト復元
