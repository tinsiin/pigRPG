...
\*\*Location:\*\* `.codex/skills/agent-memory/memories/`

\*\*Scope:\*\* Project-local. Do not store memories in global `.claude/...` paths for this repo.
...
ls .codex/skills/agent-memory/memories/



\# 2. View all summaries

rg "^summary:" .codex/skills/agent-memory/memories/ --no-ignore --hidden



\# 3. Search summaries for keyword

rg "^summary:.\*keyword" .codex/skills/agent-memory/memories/ --no-ignore --hidden -i



\# 4. Search by tag

rg "^tags:.\*keyword" .codex/skills/agent-memory/memories/ --no-ignore --hidden -i



\# 5. Full-text search (when summary search isn't enough)

rg "keyword" .codex/skills/agent-memory/memories/ --no-ignore --hidden -i
...
mkdir -p .codex/skills/agent-memory/memories/category-name/

\# Note: Check if file exists before writing to avoid accidental overwrites

cat > .codex/skills/agent-memory/memories/category-name/filename.md << 'EOF'
...
&nbsp; trash .codex/skills/agent-memory/memories/category-name/filename.md

&nbsp; # Remove empty category folders

&nbsp; rmdir .codex/skills/agent-memory/memories/category-name/ 2>/dev/null || true
